...
开场插画 origin_story → 自由走动 + 箭头点露穗 → 露穗唤醒（wake_farm）
  → 镂空教程（任务→手→拔草）→ 露穗领后续（领奖后下一句）
  → 浇水 → 回小屋睡第一夜 → 次日收获 → 路牌进镇
...
| `quest_1002`…`1005` | 领奖后进入对应任务 | 露穗口述下一动作；浇完后说今晚睡一觉 |
| `quest_1035` | 领取 1005 后 | 点小屋门睡到明天 |
| `quest_1006`…`1007` / `1009` | 领奖后进入 | 次日收获；其后斧头/鱼竿；钓完后指路进镇 |
...
| `oreshop_pass` | 矿脉商会（主线 1024） | 掌柜·赤铜放行浅层矿洞 |
| `spring_light` | 社区中心（主线 1027） | 交付铜矿、点亮春厅 |
| `ch2_done` | 领取 1027 后 | 镇长信件：下一目标是修南路去海滩（章节点，不是终局） |
