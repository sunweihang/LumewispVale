...
/** Hotbar dock face — shared tone for panel fills + inventory slot wells. */
export const UI_DOCK_FACE = new Color(210, 168, 112, 255);
/** Panel / bag / shop parchment — match hotbar dock (was too pale). */
export const UI_PARCHMENT = new Color(210, 168, 112, 255);
export const UI_PARCHMENT_LIGHT = new Color(218, 176, 120, 255);
export const UI_PARCHMENT_ROW = new Color(200, 158, 100, 255);
...
/** Drop AI plate child / host Sprite so Graphics is the only chrome renderer. */
function stripAiChrome(node: Node) {
    const plate = node.getChildByName('AiPlate');
    if (plate?.isValid) plate.destroy();
    const sp = node.getComponent(Sprite);
    if (sp) sp.destroy();
}

function ensureGraphics(node: Node, w?: number, h?: number): Graphics {
    const ut = node.getComponent(UITransform) ?? node.addComponent(UITransform);
    if (w && h && w > 0 && h > 0) ut.setContentSize(w, h);
    stripAiChrome(node);
...
/** Hotbar dock plate. */
export function drawHotbarBg(g: Graphics, w: number, h: number) {
    const x0 = -w * 0.5;
    const y0 = -h * 0.5;
    const r = 16;
    g.clear();
    ...
    g.fill();
    g.fillColor = UI_WOOD_DARK;
    g.roundRect(x0 + 5, y0 + 5, w - 10, h - 10, Math.max(4, r - 4));
    g.fill();
    g.fillColor = UI_DOCK_FACE;
    g.roundRect(x0 + 10, y0 + 10, w - 20, h - 20, Math.max(4, r - 6));
    g.fill();
    g.strokeColor = UI_STROKE;
    g.lineWidth = 3;
    g.roundRect(x0, y0, w, h, r);
    ...
    g.strokeColor = UI_GOLD;
    g.lineWidth = 2;
    g.roundRect(x0 + 6, y0 + 6, w - 12, h - 12, Math.max(4, r - 3));
    g.stroke();
}

/** Inventory / hotbar slot well (fallback when ui-slot sprite missing). */
export function drawSlotPlate(g: Graphics, size: number) {
    const half = size * 0.5;
    ...
    g.fill();
    g.fillColor = UI_DOCK_FACE;
    g.roundRect(-half + 4, -half + 4, size - 8, size - 8, Math.max(2, r - 2));
    g.fill();
    g.strokeColor = UI_STROKE;
    g.lineWidth = 2;
    g.roundRect(-half, -half, size, size, r);
...
/** Tooltip bubble (no tail — caller may add Graphics tail separately). */
export function drawTipBubble(g: Graphics, w: number, h: number) {
    const x0 = -w * 0.5;
    const y0 = -h * 0.5;
    const r = 18;
    g.clear();
    g.fillColor = new Color(253, 251, 245, 255);
    g.roundRect(x0, y0, w, h, r);
    g.fill();
    g.strokeColor = new Color(36, 34, 30, 255);
    g.lineWidth = 3;
    g.roundRect(x0, y0, w, h, r);
    g.stroke();
}
...
/** Node-facing wrappers — same look as draw*, used by prefab Chrome hosts. */
export function applyWoodPanel(node: Node, w?: number, h?: number) {
    const ut = node.getComponent(UITransform);
    const ww = w && w > 0 ? w : (ut?.contentSize.width ?? 0);
    const hh = h && h > 0 ? h : (ut?.contentSize.height ?? 0);
    if (ww <= 0 || hh <= 0) return;
    drawWoodParchmentPanel(ensureGraphics(node, ww, hh), ww, hh);
}
...
export function applyWoodButton(
    ...
    const hh = h && h > 0 ? h : (ut?.contentSize.height ?? 0);
    if (ww <= 0 || hh <= 0) return;
    drawWoodButton(ensureGraphics(node, ww, hh), ww, hh, kind);
}
...
export function applyParchmentRow(node: Node, w?: number, h?: number) {
    ...
    const hh = h && h > 0 ? h : (ut?.contentSize.height ?? 0);
    if (ww <= 0 || hh <= 0) return;
    drawParchmentRow(ensureGraphics(node, ww, hh), ww, hh);
}
...
export function applyDialogueChrome(node: Node, w?: number, h?: number) {
    ...
    const hh = h && h > 0 ? h : (ut?.contentSize.height ?? 0);
    if (ww <= 0 || hh <= 0) return;
    drawDialogueChrome(ensureGraphics(node, ww, hh), ww, hh);
}
...
export function applyTipBubble(node: Node, w?: number, h?: number) {
    ...
    const hh = h && h > 0 ? h : (ut?.contentSize.height ?? 0);
    if (ww <= 0 || hh <= 0) return;
    drawTipBubble(ensureGraphics(node, ww, hh), ww, hh);
}
...
export function applyHotbarBg(node: Node, w?: number, h?: number) {
    ...
    const hh = h && h > 0 ? h : (ut?.contentSize.height ?? 0);
    if (ww <= 0 || hh <= 0) return;
    drawHotbarBg(ensureGraphics(node, ww, hh), ww, hh);
}

export function applySlotPlate(node: Node, w?: number, h?: number) {
    const ut = node.getComponent(UITransform);
    const size = w && w > 0 ? w : h && h > 0 ? h : (ut?.contentSize.width ?? 0);
    if (size <= 0) return;
    drawSlotPlate(ensureGraphics(node, size, size), size);
}
