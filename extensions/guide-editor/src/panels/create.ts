...
module.exports = Editor.Panel.define({
  ...
  ready(this: { $: Record<string, HTMLElement | undefined> }) {
    const idInput = this.$.guideId as HTMLInputElement | undefined;
    ...
    const close = () => {
      try {
        Editor.Panel.close?.('guide-editor.create');
      } catch {
        /* ignore */
      }
    };
    ...
    const submit = async () => {
      const guideId = Number(String(idInput?.value ?? '').trim());
      ...
      if (!result?.ok) {
        await Editor.Dialog.error(result?.error || '创建失败', { title: '创建引导', buttons: ['确定'], default: 0 });
        return;
      }
      try {
