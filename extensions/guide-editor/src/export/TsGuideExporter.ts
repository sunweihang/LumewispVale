...
import { ENTRANCE_TYPE } from '../nodes/guideNodes';
import {
  CLASS_PREFIX,
  ensureDir,
...
import { validateGraphJSON } from '../validateGuideGraph';
...
export function exportGuideTs(guideId: number): ExportResult {
  syncGuideGraphOnDisk(guideId);

  const graphText = readFsText(graphFsPath(guideId));
  if (!graphText) {
    return { ok: false, guideId, error: `找不到图: ${graphFsPath(guideId)}` };
  }
  ...
  const validation = validateGraphJSON(graph, guideId);
  if (!validation.ok) {
  ...
  if (!entrance) {
    return { ok: false, guideId, error: `缺少 ${ENTRANCE_TYPE}` };
  }

  const ctx = new ExportContext(graph, guideId);
  ctx.collectReachable(entrance);

  const classTpl = loadTemplate('GuideClass.ts.tpl');
  const entranceTpl = loadTemplate('GuideEntranceBlueprint.ts.tpl');
  ...
  const className = `${CLASS_PREFIX}${guideId}`;
  ...
  const outPath = generatedClassFsPath(guideId);
...
export function exportAllFlagged(): { ok: number; fail: number; results: ExportResult[] } {
  const items = listLocalGuides().filter((s) => s.exportFlag !== false);
...
function touchExportFlag(guideId: number): void {
  const p = indexFsPath(guideId);
  if (!fs.existsSync(p)) return;
  try {
    const index = JSON.parse(fs.readFileSync(p, 'utf8')) as GuideIndexJSON;
