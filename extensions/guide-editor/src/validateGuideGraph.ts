...
export function validateGraphJSON(graph: NodeGraphJSON | null, guideId?: number): ValidateResult {
  const errors: string[] = [];
...
export function validateGuideOnDisk(guideId: number): ValidateResult {
  const text = readFsText(graphFsPath(guideId));
  if (!text) {
    return { ok: false, errors: [`找不到图: ${graphFsPath(guideId)}`], warnings: [] };
  }
  try {
    return validateGraphJSON(JSON.parse(text) as NodeGraphJSON, guideId);
  } catch (e) {
