...
exports.syncGuideGraphOnDisk = syncGuideGraphOnDisk;
...
function syncGraphPortsFromDefs(graph) {
    let changed = false;
    const defByType = new Map((0, guideNodes_1.allGuideRegisterNodes)().map((d) => [d.typeName, d]));
...
function syncGraphProfileFromDefs(graph) {
    var _a;
    const desired = (0, profile_1.buildGuideGraphProfile)();
...
function syncGuideGraphOnDisk(guideId) {
    const p = (0, paths_1.graphFsPath)(guideId);
    if (!fs.existsSync(p))
        return null;
    let graph;
    try {
...
//# sourceMappingURL=syncGuideGraphPorts.js.map