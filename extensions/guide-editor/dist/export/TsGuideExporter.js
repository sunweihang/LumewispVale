...
exports.exportGuideTs = exportGuideTs;
...
function exportGuideTs(guideId) {
    (0, syncGuideGraphPorts_1.syncGuideGraphOnDisk)(guideId);
    const graphText = (0, assetIo_1.readFsText)((0, paths_1.graphFsPath)(guideId));
    if (!graphText) {
        return { ok: false, guideId, error: `找不到图: ${(0, paths_1.graphFsPath)(guideId)}` };
    }
    let graph;
    try {
        ...
        return { ok: false, guideId, error: `图 JSON 解析失败: ${e}` };
    }
    const validation = (0, validateGuideGraph_1.validateGraphJSON)(graph, guideId);
    if (!validation.ok) {
        return { ok: false, guideId, error: validation.errors.join('; '), warnings: validation.warnings };
    }
    const entrance = graph.nodes.find((n) => n.typeName === guideNodes_1.ENTRANCE_TYPE);
    if (!entrance) {
        return { ok: false, guideId, error: `缺少 ${guideNodes_1.ENTRANCE_TYPE}` };
    }
    const ctx = new ExportContext_1.ExportContext(graph, guideId);
    ctx.collectReachable(entrance);
    const classTpl = (0, TemplateRenderer_1.loadTemplate)('GuideClass.ts.tpl');
    const entranceTpl = (0, TemplateRenderer_1.loadTemplate)('GuideEntranceBlueprint.ts.tpl');
    if (!classTpl || !entranceTpl) {
        return {
            ok: false,
            guideId,
            error: '缺少模板 GuideClass.ts.tpl 或 GuideEntranceBlueprint.ts.tpl',
        };
    }
    const fields = writeFields(ctx);
    const lifecycle = writeLifecycle(ctx, entranceTpl);
    if (lifecycle == null) {
        return { ok: false, guideId, error: '生命周期模板渲染失败' };
    }
    const methods = writeNodeMethods(ctx);
    const className = `${paths_1.CLASS_PREFIX}${guideId}`;
    ...
    const outPath = (0, paths_1.generatedClassFsPath)(guideId);
    (0, assetIo_1.writeFsText)(outPath, code);
    touchExportFlag(guideId);
...
function exportAllFlagged() {
    const items = (0, browseGuides_1.listLocalGuides)().filter((s) => s.exportFlag !== false);
    const results = [];
    let ok = 0;
    let fail = 0;
    for (const s of items) {
        const r = exportGuideTs(s.guideId);
        results.push(r);
        if (r.ok)
            ok++;
        else
...
function touchExportFlag(guideId) {
    const p = (0, paths_1.indexFsPath)(guideId);
    if (!fs.existsSync(p))
        return;
    try {
...
function writeLifecycle(ctx, entranceTpl) {
    ...
    for (const spec of specs) {
        ...
        if (!block) {
            console.error(`[guide-editor] missing lifecycle block: ${spec.methodName}`);
            return null;
        }
...
function writeNodeMethods(ctx) {
    ...
    for (const info of ctx.visitedNodes) {
        if (info.node.typeName === guideNodes_1.ENTRANCE_TYPE)
            continue;
        const tpl = (0, TemplateRenderer_1.findNodeTemplate)(info.node.typeName);
        // Guide resolve is sync — never emit await in node methods / flow calls.
        const ph = ctx.buildNodePlaceholders(info, false);
...
//# sourceMappingURL=TsGuideExporter.js.map