...
class ExportContext {
    ...
    findEntrance() {
        var _a;
        return (_a = this.graph.nodes.find((n) => n.typeName === guideNodes_1.ENTRANCE_TYPE)) !== null && _a !== void 0 ? _a : null;
    }
    collectReachable(entrance) {
        ...
        while (queue.length > 0) {
            ...
            if (!node || node.typeName === guideNodes_1.ENTRANCE_TYPE)
                continue;
            ...
            for (let i = 0; i < node.inputs.length; i++) {
                ...
                for (const c of this.inConns(node.id, i)) {
                    ...
                    if (!from || from.typeName === guideNodes_1.ENTRANCE_TYPE)
                        continue;
                    if (!seen.has(from.id)) {
                        seen.add(from.id);
                        queue.push(from.id);
    ...
    buildNodePlaceholders(info, awaitCalls = true) {
        ...
        const fields = {};
        const def = (0, guideNodes_1.findGuideNodeDef)(node.typeName);
