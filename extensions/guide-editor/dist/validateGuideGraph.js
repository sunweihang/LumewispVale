...
exports.validateGuideOnDisk = validateGuideOnDisk;
const assetIo_1 = require("./assetIo");
const guideNodes_1 = require("./nodes/guideNodes");
const paths_1 = require("./paths");
function validateGraphJSON(graph, guideId) {
    const errors = [];
    const warnings = [];
    if (!graph) {
        errors.push(guideId != null ? `引导 ${guideId} 图不存在或无法解析` : '图为空');
        return { ok: false, errors, warnings };
    }
    const entrances = graph.nodes.filter((n) => n.typeName === guideNodes_1.ENTRANCE_TYPE);
    if (entrances.length === 0) {
        errors.push(`缺少入口节点 ${guideNodes_1.ENTRANCE_TYPE}`);
    }
...
function validateGuideOnDisk(guideId) {
    const text = (0, assetIo_1.readFsText)((0, paths_1.graphFsPath)(guideId));
    if (!text) {
        return { ok: false, errors: [`找不到图: ${(0, paths_1.graphFsPath)(guideId)}`], warnings: [] };
    }
    try {
        return validateGraphJSON(JSON.parse(text), guideId);
    }
    catch (e) {
        return { ok: false, errors: [`图 JSON 解析失败: ${e}`], warnings: [] };
    }
}
//# sourceMappingURL=validateGuideGraph.js.map