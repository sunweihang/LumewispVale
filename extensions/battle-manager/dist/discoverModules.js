...
exports.discoverBattleModules = discoverBattleModules;
...
function normalizeInfoList(raw) {
    if (!raw)
        return [];
...
function withDefaultGroup(info) {
    ...
    if (info.id.startsWith('unit')) {
        return {
            ...
            groupOrder: (_a = info.groupOrder) !== null && _a !== void 0 ? _a : 10,
        };
    }
...
function listCandidatePackageNames() {
    var _a, _b, _c;
    const root = path.join(getProjectRoot(), 'extensions');
    const names = new Set(Object.keys(FALLBACK_MODULES));
    if (fs.existsSync(root)) {
        for (const name of fs.readdirSync(root)) {
            if (name === 'battle-manager' || name.startsWith('.'))
                continue;
            ...
            try {
                ...
                const msgs = ((_a = pkg.contributions) === null || _a === void 0 ? void 0 : _a.messages) || {};
                const flagged = ((_c = (_b = pkg.contributions) === null || _b === void 0 ? void 0 : _b.battleManager) === null || _c === void 0 ? void 0 : _c.enabled) === true;
...
async function packageAlive(pkgName, listMessage) {
    try {
...
function mergeFallbackFields(pkgName, infos) {
    ...
    return infos.map((info) => {
        ...
        return {
            ...
            messages: {
                ...
                validateOne: info.id === 'unit' ? undefined : info.messages.validateOne || fb.messages.validateOne,
            },
            extraActions: ((_a = info.extraActions) === null || _a === void 0 ? void 0 : _a.length) ? info.extraActions : fb.extraActions,
        };
    });
}
async function resolveModules(pkgName) {
    // 1) 优先正式契约（支持单对象或数组）
    try {
        ...
        if (msg.includes('Message does not exist')) {
            console.warn(`[battle-manager] ${pkgName} 未注册 battle-module-info（扩展可能未重载），改用内置契约`);
        }
        else {
...
async function discoverBattleModules() {
    ...
    modules.sort((a, b) => {
        var _a, _b;
        return ((_a = a.groupOrder) !== null && _a !== void 0 ? _a : 50) - ((_b = b.groupOrder) !== null && _b !== void 0 ? _b : 50) ||
            a.order - b.order ||
            a.title.localeCompare(b.title, 'zh');
    });
    console.log(`[battle-manager] discovered ${modules.length} module(s):`, modules.map((m) => `${m.groupTitle || m.group}/${m.title}(${m.packageName})`).join(', ') ||
        '(none)');
    return modules;
}
//# sourceMappingURL=discoverModules.js.map