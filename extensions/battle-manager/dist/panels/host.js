                    ...
                    (gid === 'unit'
                        ...
                        : gid === 'effect'
                            ...
                            : gid === 'scene'
                                ? '场景管理'
                                : '战斗管理器'),
                order: (_a = m.groupOrder) !== null && _a !== void 0 ? _a : (gid === 'unit' ? 10 : gid === 'effect' ? 12 : gid === 'scene' ? 15 : 20),
                modules: [],
            };
            map.set(gid, g);
        }
...
function normalizeList(mod, raw) {
    if (!Array.isArray(raw))
        return [];
    return raw.map((row, index) => {
        var _a, _b, _c;
        const r = row;
        const id = Number((_b = (_a = r[mod.itemIdKey]) !== null && _a !== void 0 ? _a : r.id) !== null && _b !== void 0 ? _b : index + 1);
        const name = String((_c = r.name) !== null && _c !== void 0 ? _c : `${mod.title} ${id}`);
        ...
        if (!subtitle) {
            const flags = [];
            if (r.exportFlag !== undefined)
                flags.push(r.exportFlag !== false ? '导出' : '未导出');
            if (r.hasGraph !== undefined)
                flags.push(r.hasGraph ? '有图' : '无图');
            if (typeof r.unitCount === 'number')
                flags.push(`${r.unitCount} 个单位`);
            if (typeof r.prefab === 'string' && r.prefab)
                flags.push(String(r.prefab));
            subtitle = flags.join(' · ');
        }
        return {
            id,
...
async function loadModules(preferId) {
    ...
    try {
        const state = (await Editor.Message.request(PKG, 'query-host-state'));
        modules = Array.isArray(state === null || state === void 0 ? void 0 : state.modules) ? state.modules : [];
        if (!selectId && (state === null || state === void 0 ? void 0 : state.selectId))
            selectId = state.selectId;
    }
    catch {
        try {
            const res = (await Editor.Message.request(PKG, 'query-modules'));
    ...
    if (selectId && modules.some((m) => m.id === selectId)) {
        ...
        activeId = (_b = (_a = modules[0]) === null || _a === void 0 ? void 0 : _a.id) !== null && _b !== void 0 ? _b : null;
    }
    renderShell();
    await refreshList();
}
function renderShell() {
    ...
    const title = rootEl.querySelector('#mod-title');
    if (!nav || !title)
        return;
    nav.innerHTML = '';
    if (modules.length === 0) {
    ...
    for (const g of groups) {
        ...
        for (const m of g.modules) {
            ...
            btn.addEventListener('click', async () => {
                activeId = m.id;
                searchQuery = '';
                const searchInput = rootEl === null || rootEl === void 0 ? void 0 : rootEl.querySelector('#search-input');
                if (searchInput)
                    searchInput.value = '';
                try {
    ...
    const groupTitle = (mod === null || mod === void 0 ? void 0 : mod.groupTitle) ||
        ((mod === null || mod === void 0 ? void 0 : mod.group) === 'unit'
            ? '单位管理器'
            : (mod === null || mod === void 0 ? void 0 : mod.group) === 'effect'
                ? '特效管理器'
                : (mod === null || mod === void 0 ? void 0 : mod.group) === 'scene'
                    ? '场景管理'
                    : '战斗管理器');
    title.textContent = mod ? `${groupTitle} · ${mod.title}` : 'Game编辑器';
    const batchBtn = rootEl.querySelector('#btn-batch');
    const createBtn = rootEl.querySelector('#btn-create');
    if (batchBtn) {
        // 单位管理不提供「批量扫描挂点」；技能等模块仍可批量导出
        const hideBatch = !!(mod === null || mod === void 0 ? void 0 : mod.hideExport) || (mod === null || mod === void 0 ? void 0 : mod.id) === 'unit';
        batchBtn.style.display = hideBatch ? 'none' : '';
        batchBtn.textContent = (mod === null || mod === void 0 ? void 0 : mod.exportLabel) ? `批量${mod.exportLabel}` : '批量导出';
    }
    if (createBtn) {
        createBtn.style.display = (mod === null || mod === void 0 ? void 0 : mod.hideCreate) ? 'none' : '';
    }
    const listHeader = rootEl.querySelector('#list-header');
    if (listHeader) {
        if ((mod === null || mod === void 0 ? void 0 : mod.id) === 'unit') {
...
function renderListRows(mod, items) {
    const listEl = rootEl === null || rootEl === void 0 ? void 0 : rootEl.querySelector('#list');
    if (!listEl)
        return;
    ...
    for (const item of filtered) {
        ...
        const extras = (mod.extraActions || [])
            ...
            .map((a) => `<button data-act="extra" data-extra="${escapeHtml(a.id)}" data-id="${item.id}">${escapeHtml(a.label)}</button>`)
            .join('');
        // 单位管理不用「校验」；技能/弹道等仍可用
        const validateBtn = mod.id !== 'unit' && mod.messages.validateOne
            ? `<button data-act="validate" data-id="${item.id}">校验</button>`
            : '';
        const locateBtn = mod.id === 'unit' || mod.messages.locate
        ...
        <span class="col-category"><span class="category" title="${escapeHtml(otherText)}">${escapeHtml(otherText)}</span></span>
      `;
        row.innerHTML = `
    ...
    listEl.querySelectorAll('button').forEach((btn) => {
        btn.addEventListener('click', async () => {
            ...
            if (act === 'toggle-ops') {
                ...
                const btns = rowEl === null || rowEl === void 0 ? void 0 : rowEl.querySelector('.action-btns');
                const willExpand = !expandedActionRows.has(key);
                if (willExpand)
                    expandedActionRows.add(key);
                else
                    expandedActionRows.delete(key);
                btns === null || btns === void 0 ? void 0 : btns.classList.toggle('hidden', !willExpand);
                btn.textContent = willExpand ? '隐藏操作' : '显示操作';
                return;
            }
            ...
            try {
                if (act === 'edit') {
                    ...
                    const extra = (_a = mod.extraActions) === null || _a === void 0 ? void 0 : _a.find((a) => a.id === extraId);
                    if (extra) {
...
module.exports = Editor.Panel.define({
  ...
  `,
    ...
    methods: {
        async panelSetModule(arg) {
            var _a;
            void (arg === null || arg === void 0 ? void 0 : arg.rescan);
            await loadModules((_a = arg === null || arg === void 0 ? void 0 : arg.moduleId) !== null && _a !== void 0 ? _a : activeId);
        },
        getActiveModuleId() {
            return activeId;
        },
    ...
    ready() {
        var _a, _b, _c;
        rootEl = this.$.root;
        (_a = this.$.refresh) === null || _a === void 0 ? void 0 : _a.addEventListener('click', () => refreshList());
        (_b = this.$.create) === null || _b === void 0 ? void 0 : _b.addEventListener('click', async () => {
            const mod = activeModule();
            if (!mod || mod.hideCreate)
                return;
