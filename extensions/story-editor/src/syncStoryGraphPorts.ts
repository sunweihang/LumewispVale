...
import { allStoryRegisterNodes } from './nodes/storyNodes';
...
export function syncGraphPortsFromDefs(graph: NodeGraphJSON): boolean {
  let changed = false;
  const defByType = new Map(allStoryRegisterNodes().map((d) => [d.typeName, d]));
...
export function syncGraphProfileFromDefs(graph: NodeGraphJSON): boolean {
  const desired = buildStoryGraphProfile();
...
export function syncStoryGraphOnDisk(storyId: number): NodeGraphJSON | null {
  const p = graphFsPath(storyId);
