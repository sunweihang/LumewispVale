...
export function validateGraphJSON(graph: NodeGraphJSON | null, storyId?: number): ValidateResult {
  const errors: string[] = [];
...
export function validateStoryOnDisk(storyId: number): ValidateResult {
  const text = readFsText(graphFsPath(storyId));
  if (!text) {
    return { ok: false, errors: [`找不到图: ${graphFsPath(storyId)}`], warnings: [] };
  }
  try {
    return validateGraphJSON(JSON.parse(text) as NodeGraphJSON, storyId);
  } catch (e) {
