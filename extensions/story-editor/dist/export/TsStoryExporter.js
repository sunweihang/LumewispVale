...
exports.exportStoryTs = exportStoryTs;
...
function exportStoryTs(storyId) {
    (0, syncStoryGraphPorts_1.syncStoryGraphOnDisk)(storyId);
    const graphText = (0, assetIo_1.readFsText)((0, paths_1.graphFsPath)(storyId));
    if (!graphText) {
        return { ok: false, storyId, error: `找不到图: ${(0, paths_1.graphFsPath)(storyId)}` };
    }
    let graph;
    try {
    ...
    const validation = (0, validateStoryGraph_1.validateGraphJSON)(graph, storyId);
    if (!validation.ok) {
        return { ok: false, storyId, error: validation.errors.join('; '), warnings: validation.warnings };
    }
    const entrance = graph.nodes.find((n) => n.typeName === storyNodes_1.ENTRANCE_TYPE);
    if (!entrance) {
        return { ok: false, storyId, error: `缺少 ${storyNodes_1.ENTRANCE_TYPE}` };
    }
    const ctx = new ExportContext_1.ExportContext(graph, storyId);
    ctx.collectReachable(entrance);
    const classTpl = (0, TemplateRenderer_1.loadTemplate)('StoryClass.ts.tpl');
    const entranceTpl = (0, TemplateRenderer_1.loadTemplate)('StoryEntranceBlueprint.ts.tpl');
    ...
    const className = `${paths_1.CLASS_PREFIX}${storyId}`;
    ...
    const outPath = (0, paths_1.generatedClassFsPath)(storyId);
...
function exportAllFlagged() {
    const items = (0, browseStories_1.listLocalStories)().filter((s) => s.exportFlag !== false);
    const results = [];
    let ok = 0;
    let fail = 0;
...
function touchExportFlag(storyId) {
    const p = (0, paths_1.indexFsPath)(storyId);
    if (!fs.existsSync(p))
        return;
    try {
...
function writeNodeMethods(ctx) {
    ...
    for (const info of ctx.visitedNodes) {
        if (info.node.typeName === storyNodes_1.ENTRANCE_TYPE)
            continue;
...
//# sourceMappingURL=TsStoryExporter.js.map