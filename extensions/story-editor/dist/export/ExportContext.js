...
class ExportContext {
    ...
    findEntrance() {
        var _a;
        return (_a = this.graph.nodes.find((n) => n.typeName === storyNodes_1.ENTRANCE_TYPE)) !== null && _a !== void 0 ? _a : null;
    }
    collectReachable(entrance) {
        ...
        while (queue.length > 0) {
            ...
            if (!node || node.typeName === storyNodes_1.ENTRANCE_TYPE)
                continue;
            ...
            for (let i = 0; i < node.inputs.length; i++) {
                ...
                for (const c of this.inConns(node.id, i)) {
                    ...
                    if (!from || from.typeName === storyNodes_1.ENTRANCE_TYPE)
                        continue;
                    if (!seen.has(from.id)) {
                        seen.add(from.id);
                        queue.push(from.id);
    ...
    dataPreludeLines(node, awaitCalls) {
        const lines = [];
        const need = new Set();
        for (let i = 0; i < node.inputs.length; i++) {
            if (node.inputs[i].portType === FLOW)
        ...
        for (const id of need) {
            ...
            if (!v)
                continue;
            if (awaitCalls)
                lines.push(`await this.${v.methodName}();`);
            else
                lines.push(`void this.${v.methodName}();`);
        }
        return lines;
    }
    ...
    buildNodePlaceholders(info, awaitCalls = true) {
        ...
        const fields = {};
        const def = (0, storyNodes_1.findStoryNodeDef)(node.typeName);
